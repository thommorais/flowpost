...
const Card = ({ className, ...props }: ComponentPropsWithoutRef<'div'>) => {
	return (
		<div
			className={clsx(
				'w-full rounded-xl border border-charcoal-blue/30 bg-charcoal-blue-600/15 p-8 shadow-2xl',
				className,
			)}
			{...props}
		/>
...
export const FlagsGameClient = () => {
	...
	const persistScoreSafely = useCallback(async (nextScore: ScoreSnapshot) => {
		try {
			...
			console.warn('IndexedDB persistence failed, keeping in memory', error)
	...
	const persistSessionSafely = useCallback(async (nextSession: GameSession) => {
		try {
			...
			console.warn('Session persistence failed, keeping in memory', error)
		}
	}, [])
	...
	useEffect(() => {
		...
		;(async () => {
			try {
				...
				console.warn('Unable to hydrate data from IndexedDB', error)
				if (!cancelled) {
	...
	}, [focusOption, focusedIndex, handleNextRound, handleSkip, statusRef.current])
		...
		return (
			<div className='flex min-h-dvh items-center justify-center px-4'>
				<Card className='max-w-md'>
					...
					<p className='mb-6 text-center text-charcoal-blue-600'>{t('test_geography_knowledge')}</p>

					<div className='mb-6'>
						<label htmlFor='rounds' className='mb-2 block font-semibold text-charcoal-blue-600 text-sm'>
					...
					<button
						type='button'
						className='w-full rounded-lg bg-pollen-500 px-6 py-3 font-semibold text-pollen-200 shadow-md transition hover:translate-y-[-2px] hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-pumpkin-spice-600/75'
		...
		return (
			<div className='flex min-h-dvh items-center justify-center px-4'>
				<Card className='w-full max-w-md rounded-xl p-8 shadow-lg'>
					...
					<div className='mb-6 rounded-lg bg-charcoal-blue-900/50 p-6 text-center text-charcoal-blue-100'>
					...
					<div className='mb-6 space-y-2 text-charcoal-blue-100'>
						<div className='flex justify-between rounded-lg bg-charcoal-blue-900/50 px-4 py-3'>
					...
					<button
						type='button'
						className='w-full rounded-lg bg-pollen px-6 py-3 font-semibold text-pollen-100 transition hover:translate-y-[-2px] hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-pumpkin-spice-600/75'
		...
		<div className='mx-auto grid max-w-4xl grid-cols-1 place-items-start gap-4 px-4 py-8 lg:grid-cols-12'>
			<Card className='flex flex-col gap-4 p-4 lg:col-span-4 lg:max-w-xs'>
				<header className='flex items-center justify-between'>
					<div>
						<p className='font-semibold text-charcoal-blue-700 text-sm uppercase tracking-wide'>{t('scoreboard')}</p>
					</div>
				...
				{persistenceWarning ? (
					<p className='rounded-lg border border-amber-300 bg-amber-50 px-3 py-2 text-amber-800 text-xs'>
						{persistenceWarning}
					</p>
				) : null}
				{loadingScore ? <p className='text-stone-500 text-xs'>{t('loading_saved_progress')}</p> : null}
			</Card>

			<Card className='flex flex-1 flex-col gap-4 p-4 lg:col-span-8'>
				{session.mode === 'playing' && (
					<div className='rounded-lg border border-charcoal-blue-100/75 p-3 px-3 py-2'>
						<p className='mb-1 font-semibold text-xs uppercase tracking-wide'>{t('session_progress')}</p>
						<div className='flex items-center gap-2'>
							<div className='h-2 flex-1 overflow-hidden rounded-full bg-charcoal-blue-100'>
								<div
						...
						<div className='mt-2 flex items-center justify-end gap-3 text-xs'>
							...
							<span className='text-pumpkin-spice-600'>
				...
				<div
					ref={flagCardRef}
					className='relative aspect-video w-full overflow-hidden rounded-lg bg-charcoal-blue-900/80'
				>
					<span
					...
					{flagError ? (
						<div className='absolute inset-0 flex flex-col items-center justify-center gap-3 bg-white/80 text-center text-stone-700-sm'>
							...
							<button
								type='button'
								className='rounded-full border bg-charcoal-blue-700 px-4 py-2 font-semibold text-sm transition hover:shadow-sm focus:outline-none focus:ring-2 focus:ring-pumpkin-spice-600/75'
								onClick={() => {
									setFlagError(false)
								}}
							>
				...
				<div className='min-h-18 rounded-lg bg-white-50/50 p-3' aria-live='polite'>
				...
				<div className='grid grid-cols-1 gap-3 sm:grid-cols-2'>
					{round.options.map((option, index) => {
						...
						const baseClasses =
							'flex h-full w-full items-center gap-3 rounded-full px-4 py-3 text-left text-sm font-semibold transition focus:outline-none focus:ring-2 focus:ring-pumpkin-spice-600/75'
						const playingClasses =
							'border border-charcoal-blue-800/50 text-charcoal-blue-800 hover:-translate-y-0.5 hover:shadow-sm'
						const correctClasses = ' bg-olive-400 text-olive-900'
						const incorrectClasses = 'bg-pumpkin-spice-400 text-pumpkin-spice-900'
						...
						return (
							...
							>
								<span className='flex h-8 w-8 items-center justify-center rounded-full bg-charcoal-blue-800/25 font-bold text-xs'>
									{index + 1}
								</span>
				...
				<div className='mt-6 flex w-full flex-col gap-3 sm:flex-row sm:items-center sm:justify-between'>
					<button
						type='button'
						className='rounded-lg px-4 py-2 font-semibold text-sm shadow-sm transition hover:translate-y-[-1px] hover:shadow focus:outline-none focus:ring-2 focus:ring-pumpkin-spice-600/75 disabled:cursor-not-allowed disabled:opacity-60'
						onClick={handleSkip}
...
const Stat = ({ label, value }: StatProps): React.ReactElement => {
	return (
		<div className='rounded-lg border border-charcoal-blue-100/75 px-3 py-2 opacity-75'>
			<p className='font-semibold text-charcoal-blue-900/75 text-xs uppercase tracking-wide'>{label}</p>
